...
path = str('/Users/arshmacbook/Desktop/36-614/data_engineering_project/\
    hhs_weekly_data_files/')  # Change this to the user directory
    ...
    for column, row in hhs.iterrows():
        try:
            with conn.transaction():
                ...
                # If a hospital already exists in the table then update values
                # in the address table
                cur.execute("SELECT hospital_pk FROM address")
                existing_hospitals = np.array(
                    [elem[0] for elem in pd.Series(cur.fetchall())]
                    )
                ...
                # If the hospital does not exist in the address table then
                # insert it
                else:
...
print("Number of rows successfully inserted:", round(
    num_rows_successfully_inserted_hhs / hhs.shape[0] * 100, 2
    ), "%\n")
print("Number of rows unable to be inserted due to errors:", round(
    num_rows_error_hhs / hhs.shape[0] * 100, 2
    ), "%\n")
